...
import dev.jombi.kordsb.rest.builder.integration.IntegrationModifyBuilder
import dev.jombi.kordsb.rest.builder.member.MemberAddBuilder
...
import dev.jombi.kordsb.rest.json.request.*
...
public class GuildService(requestHandler: RequestHandler) : RestService(requestHandler) {
    ...
    @Deprecated("Binary compatibility, keep for at least one release.", level = DeprecationLevel.HIDDEN)
    ...
    @Deprecated("Guild embeds were renamed to widgets.", ReplaceWith("getGuildWidget(guildId)"), DeprecationLevel.HIDDEN)
